...
void ProgramToFile(Mat confusion,int quantidadeDeClasses,float atributos,float objetos,float acerto,float erro, int K,int decision,float perTreino,float perTeste){
    file=fopen(Resultados, "w");
    fprintf(file, "\t\tMETODO DE CLASSIFICACAO : KNN %d\n",K);
    if (decision==1) {
...
void Method_KNN(Mat Treino,Mat Teste,Mat TreinoLabel,Mat TesteLabel,Mat label,Mat atrib,float percentualTreino,float percentualTeste,int quantidadeDeClasses,int NumObjPorClasse[quantidadeDeClasses],float atributos,float objetos,Mat confusion){
    //Escolher: Leave_on_out or Hold_out(Metodos)
    int decision;
    printf("Selecione:\n1-Leave_On_Out\n2-Hold-out\n");
    scanf("%d",&decision);
    if (decision==1) {
        leave_on_out(percentualTreino, percentualTeste, quantidadeDeClasses, NumObjPorClasse, atributos, objetos, label, atrib, Treino, Teste, TreinoLabel, TesteLabel);
    }
    if (decision==2) {
        hold_out(percentualTreino, percentualTeste, quantidadeDeClasses, NumObjPorClasse, atributos, objetos, label, atrib, Treino, Teste, TreinoLabel, TesteLabel);
    }
    ...
    printf("Metodo de Classificacao KNN\n");
    ...
    for (int tsample = 0; tsample < ((int)objetos*(percentualTeste/100)-1); tsample++) {
    ...
    ProgramToFile(confusion, quantidadeDeClasses, atributos, objetos, acerto, erro, K,decision,percentualTreino,percentualTeste);
}
int main(){
    ...
    Method_KNN(perTreino, perTeste, quantidadeClasses, NumObjPorClasse, atributos, objetos, label, atrib, Treino, Teste, TreinoLabel, TesteLabel, Confusion);
}