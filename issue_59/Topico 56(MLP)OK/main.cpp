...
FILE* file;
char address[500]="/Users/wellcome/Desktop/DataBase/numbersDB/num_padronizado_GLCM_OpenCv.txt";//Banco de dados - Carregar
...
void ProgramToFile(Mat confusion,int quantidadeDeClasses,float atributos,float objetos,float acerto,float erro, int config,int decision,float perTreino,float perTeste){
    file=fopen(Resultados, "w");
    if (config==1) {
        fprintf(file, "\t\tMETODO DE CLASSIFICACAO : MLP\n\t\tCONFIGURACAO 01\n");
    }
    if (config==2) {
        fprintf(file, "\t\tMETODO DE CLASSIFICACAO : MLP\n\t\tCONFIGURACAO 02\n");
    }
...
void Method_MLP(Mat Treino,Mat Teste,Mat TreinoLabel,Mat TesteLabel,Mat label,Mat atrib,float perTreino,float perTeste,int quantidadeClasses,int NumObjPorClasse[quantidadeClasses],float atributos,float objetos,Mat Confusion){
    int decision;
    ...
    ProgramToFile(Confusion, quantidadeClasses, atributos, objetos, acerto, erro, config, decision, perTreino, perTeste);
}
int main(){
    ...
    Method_MLP(Treino, Teste, TreinoLabel, TesteLabel, label, atrib, perTreino, perTeste, quantidadeClasses, NumObjPorClasse, atributos, objetos,Confusion);
}
