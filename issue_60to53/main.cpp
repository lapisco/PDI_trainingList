...
void ProgramToFile(Mat confusion,int quantidadeClasses,float atributos,float objetos,float acerto,float erro,int decision,float perTreino,float perTeste,float speciticivy,float sensitivity,float ppv,float accuracy,Mat ResPorClasse,int rod,Mat ResMedia, int Media, double TimeTrain, double TimeTestAll, double TimeTestOne){
    ...
    fprintf(file, "\n\t\tTEMPOS\n");
    fprintf(file, "TEMPO DE TREINAMENTO : %.2f Milisegundos \n",TimeTrain);
    fprintf(file, "TEMPO DE TESTE TOTAL : %.2f Milisegundos \n",TimeTestAll);
    fprintf(file, "TEMPO DE TESTE AMOSTRA : %.2f Milisegundos \n\n",TimeTestOne);
    ...
    if (Media==9) {
        ...
        fprintf(file, "\n-------------------------------------------------------------------------------------------\n");
        fprintf(file, "-------------------------------------------------------------------------------------------\n");
        fprintf(file, "\n\t\tDESVIO PADRAO DAS MEDIDAS ESTATISTICAS\n");
        float var0=0,var1=0,var2=0,var3=0;
        for (int x=0; x<quantidadeClasses; x++) {
            for (int y=0; y<4; y++) {
                if (y==0) {
                    var0+=pow((ResMedia.at<float>(x,y)-(speciticivy/10)),2);
                }
                if (y==1) {
                    var1+=pow((ResMedia.at<float>(x,y)-(sensitivity/10)),2);
                }
                if (y==2) {
                    var2+=pow((ResMedia.at<float>(x,y)-(accuracy/10)),2);
                }
                if (y==3) {
                    var3+=pow((ResMedia.at<float>(x,y)-(ppv/10)),2);
                }
            }
        }
        fprintf(file, "\nDesvio Padrao Speciticivy - %f\n",sqrt(var0/(quantidadeClasses-1)));
        fprintf(file, "Desvio Padrao Sensitivity - %f\n",sqrt(var1/(quantidadeClasses-1)));
        fprintf(file, "Desvio Padrao Accuracy - %f\n",sqrt(var2/(quantidadeClasses-1)));
        fprintf(file, "Desvio Padrao PPV - %f\n",sqrt(var3/(quantidadeClasses-1)));
        fprintf(file, "\n-------------------------------------------------------------------------------------------\n");
        fprintf(file, "----------------------------------------END------------------------------------------------\n");
        fprintf(file, "-------------------------------------------------------------------------------------------\n\n");
...
void hold_out(float percentualTreino,float percentualTeste,int quantidadeDeClasses,int NumObjPorClasse[quantidadeDeClasses],float atributos,float objetos,Mat label,Mat atrib,Mat Treino,Mat Teste,Mat TreinoLabel,Mat TesteLabel){
    ...
    for (int controle=0; controle<quantidadeDeClasses; controle++){
        ...
        for (int tk=0; tk<NumObjPorClasse[controle]; tk++)
                vetObj.at<float>(tk)=0;
        int por=-1;
        for (int x=0; x<objetos; x++) {
        ...
        int objetovar=int(NumObjPorClasse[controle]+1);
        int contador=0;
        srand(time(NULL));
...
//                            printf("TREINO - %f\n",Treino.at<float>(contadorTreino,xx));
//                        if(xx==atributos)
//                            printf("TREINO - %f\n",TreinoLabel.at<float>(contadorTreino,xx));
                    }
                }
                else{
                    contadorTeste++;
...
//                            printf("TESTE - %f\n",Teste.at<float>(contadorTeste,xx));
//                        if(xx==atributos)
//                            printf("TESTE - %f\n",TesteLabel.at<float>(contadorTeste,xx));
                    }
                }
            }
        }
...
void Method_KNN(Mat Treino,Mat Teste,Mat TreinoLabel,Mat TesteLabel,Mat label,Mat atrib,float perTreino,float perTeste,int quantidadeClasses,int NumObjPorClasse[quantidadeClasses],float atributos,float objetos,int decision,Mat Confusion,int Media,Mat ResMedia){
    //Declaracao Tempos
    double TimeTrain,TimeTestAll,TimeTestOne;
    ...
    //Calcular Tempo Train (FIM)
    TmpTrain[1]= clock();
    TimeTrain= (TmpTrain[1]-TmpTrain[0])*1000.0/CLOCKS_PER_SEC;
    printf("\nTempo Train - %f Milisegundos\n",TimeTrain);
    ...
    //Calcular Tempo TestAll
    clock_t TmpTestAll[2];
    TmpTestAll[0] = clock();
    
    for (int tsample = 0; tsample < ((int)objetos*(perTeste/100)-1); tsample++) {
        //Calcular Tempo TestOne
        clock_t TmpTestOne[2];
        TmpTestOne[0] = clock();
        ...
        if (tsample==0) {
            //Calcular Tempo TestOne (FIM)
            TmpTestOne[1]= clock();
            TimeTestOne= (TmpTestOne[1]-TmpTestOne[0])*1000.0/CLOCKS_PER_SEC;
        }
    }
    //Calcular Tempo TestAll (FIM)
    TmpTestAll[1]= clock();
    TimeTestAll= (TmpTestAll[1]-TmpTestAll[0])*1000.0/CLOCKS_PER_SEC;
    printf("\nTempo TestOne - %f Milisegundos\n",TimeTestOne);
    printf("\nTempo TestAll - %f Milisegundos\n",TimeTestAll);
    ...
    ProgramToFile(Confusion, quantidadeClasses, atributos, objetos, acerto, erro,decision,perTreino,perTeste,speciticivy,sensitivity,ppv,accuracy,ResPorClasse,rod,ResMedia,Media,TimeTrain,TimeTestAll,TimeTestOne);
}
...
int main(){
    ...
    //Declaracao da Mat ResMedia e Mat DesvioPadrao
