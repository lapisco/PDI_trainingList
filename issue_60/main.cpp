...
//PARA ARQUIVO//
void ProgramToFile(Mat confusion,int quantidadeClasses,float atributos,float objetos,float acerto,float erro,int decision,float perTreino,float perTeste,float speciticivy,float sensitivity,float ppv,float accuracy,Mat ResPorClasse,int rod){
    if (rod==1)
...
void Method_KNN(Mat Treino,Mat Teste,Mat TreinoLabel,Mat TesteLabel,Mat label,Mat atrib,float perTreino,float perTeste,int quantidadeClasses,int NumObjPorClasse[quantidadeClasses],float atributos,float objetos,int decision,Mat Confusion){
    ...
    ProgramToFile(Confusion, quantidadeClasses, atributos, objetos, acerto, erro,decision,perTreino,perTeste,speciticivy,sensitivity,ppv,accuracy,ResPorClasse,rod);
}
